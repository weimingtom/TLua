/*************************************************************************
	> File Name: l_socket.c
	> Author:TTc 
	> Mail:liutianshxkernel@.gmail.com 
	> Created Time: 一  5/ 8 16:45:58 2017
 ************************************************************************/
 ...
 */
static lSocket*
lsocket_checklSocket(lua_State *L, int index)
{
 ...
 */
static int
lsocket_islSocket(lua_State *L, int index)
{
 ...
 */
static lSocket*
lsocket_pushlSocket(lua_State *L)
{
 ...
 */
static int
lsocket_sock__toString(lua_State *L)
{
 ...
 */
static int
lsocket_error(lua_State *L, const char *msg)
{
    lua_pushnil(L);
 ...
 */
static int
_initsocket(lSocket *sock, int family, int type, int mcast, int protocol, int listening)
{
    if (sock->sockfd == -1)
        return 0;
 ...
 */
static const char *
_addr2string(struct sockaddr *sa, socklen_t slen, char *buf, int buflen)
{
    const char *s = 0;
 ...
 */
static uint16_t
_portnumber(struct sockaddr *sa)
{
    uint16_t port = 0;
    if (sa->sa_family == AF_INET)
 ...
 */
static int
_needsnolookup(const char *addr)
{
 ...
 */
static int
_gethostaddr(lua_State *L, const char *addr, int type, int port, int *family, int *protocol, struct sockaddr *sa, socklen_t *slen)
{
    if (strchr(addr, '/') || *addr == '@') {
        if (strlen(addr) > UNIX_PATH_MAX)
            return lsocket_error(L, "unix domain path too long");
        *family = AF_UNIX;
        *slen = (int)(sizeof(sa_family_t) + strlen(addr) + 1);
        *protocol = 0;
 ...
 */
static void
_push_sockname(lua_State *L, struct sockaddr *sa, socklen_t slen)
{
 ...
 */
static int
lsocket_sock_status(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    int err;
 ...
 */
static int
lsocket_sock_getfd(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    lua_pushinteger(L, sock->sockfd);
    return 1;
 ...
 */
static int
lsocket_sock_setfd(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
 ...
 */
static int
_canacceptdata(int fd, int send)
{
    fd_set rfds;
    struct timeval tv;
    int ok = -1;
 ...
 */
static int
lsocket_sock_accept(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    char buf[SOCKADDR_BUFSIZ];
...
lsocket_sock_recv(lua_State *L)
{
    ...
    int nrd = (int)recv(sock->sockfd, buf, howmuch, 0);
    if (nrd < 0) {
 ...
 */
static int
lsocket_sock_recvfrom(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    uint32_t howmuch = luaL_optnumber(L, 2, LUAL_BUFFERSIZE);
    if (lua_tointeger(L, 2) > UINT_MAX)
    ...
    int nrd = (int)recvfrom(sock->sockfd, buf, howmuch, 0, sa, &slen);
    if (nrd < 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK)
            lua_pushboolean(L, 0);
        else
...
static int lsocket_sock_send(lua_State *L)
{
    ...
    int nwr = (int)send(sock->sockfd, data, len, flags);
 ...
 */
static int
lsocket_sock_sendto(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    size_t len;
    const char *data = luaL_checklstring(L, 2, &len);
    ...
    int nwr = (int)sendto(sock->sockfd, data, len, flags, sa, slen);
 ...
 */
static int
lsocket_sock_close(lua_State *L)
{
    lSocket *sock = lsocket_checklSocket(L, 1);
    int err = 0;
    if (sock->sockfd >= 0)
 ...
 */
static int
_table2fd_set(lua_State *L, int idx, fd_set *s)
{
    int i = 1;
    int maxfd = -1;
    lua_rawgeti(L, idx, i++);
 ...
 */
static int
_push_sock_for_fd(lua_State *L, int idx, int fd)
{
    int i = 1;
    lua_rawgeti(L, idx, i++);
    while (lsocket_islSocket(L, -1)) {
 ...
 */
static int
_fd_set2table(lua_State *L, int idx, fd_set *s, int maxfd)
{
    int i, n = 1;
    lua_newtable(L);
    for (i = 0; i <= maxfd; ++i) {
 ...
 */
static int
lsocket_getinterfaces(lua_State *L)
{
    struct ifaddrs *ifa;
    char buf[SOCKADDR_BUFSIZ];
    const char *s;
